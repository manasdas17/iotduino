...
uint16_t Adafruit_BMP085::readPressure(void) {
...
void Adafruit_BMP085::readPressure(HardwareCommandResult* hwresult)
{
...
int8_t Adafruit_BMP085::readTemperature(void) {
...
void Adafruit_BMP085::readTemperature(HardwareCommandResult* hwresult)
{
...
void Adafruit_BMP085::init(uint8_t hwaddress)
{
	I2C::init(BMP085_I2CADDR, hwaddress);
	begin();
}

boolean Adafruit_BMP085::implementsInterface(HardwareTypeIdentifier type)
{
	if(type == HWType_pressure || type == HWType_temprature)
		return true;
	return false;
}

boolean Adafruit_BMP085::readVal(HardwareTypeIdentifier type, HardwareCommandResult* result)
{
	switch(type) {
...
HardwareTypeIdentifier* Adafruit_BMP085::getImplementedInterfaces(HardwareTypeIdentifier* arr, uint8_t maxLen)
{
	addImplementedInterface(arr, maxLen, HWType_temprature);
	return addImplementedInterface(arr, maxLen, HWType_pressure);
}
